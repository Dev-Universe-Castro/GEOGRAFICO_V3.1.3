...
// Funções para gerenciamento de camadas
...
function updateActiveLayersList() {
    ...
    if (!window.activeLayers || window.activeLayers.length === 0) {
    ...
    window.activeLayers.forEach(layer => {
        ...
        const visibilityColor = layer.visible ? '#4CAF50' : '#ccc';
            ...
            <div class="layer-item" data-layer-id="${layer.id}">
                <div class="layer-item-header">
                    <div>
                        ...
                        <div class="layer-item-type">${getLayerTypeLabel(layer.type)}</div>
                    </div>
                </div>
                <div class="layer-controls">
                    <button class="btn btn-sm btn-outline-secondary" onclick="toggleLayerVisibility(${layer.id})" title="Alternar visibilidade">
                        <i class="fas ${visibilityIcon}" style="color: ${visibilityColor};"></i>
                    </button>
                    <button class="btn btn-sm btn-outline-primary" onclick="editLayer(${layer.id})" title="Editar">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn btn-sm btn-outline-danger" onclick="removeLayer(${layer.id})" title="Remover">
...
function getLayerTypeLabel(type) {
    const labels = {
        ...
        'receitas': 'Receitas',
        'revendas': 'Territórios de Revendas'
    };
...
function editLayer(layerId) {
    ...
    // Set the layer type first
    document.getElementById('layer-type-selector').value = layer.type;

    // Trigger change event to show the correct configuration panel
    const event = new Event('change');
    document.getElementById('layer-type-selector').dispatchEvent(event);

    // Wait a bit for the panel to show, then populate fields
    setTimeout(() => {
        switch (layer.type) {
            case 'crop':
...
function removeLayer(layerId) {
    if (confirm('Tem certeza que deseja remover esta camada?')) {
        ...
        // Remove from active layers array
        window.activeLayers = window.activeLayers.filter(l => l.id !== layerId);

        // Update UI
        updateActiveLayersList();
        updateCombinedLegend();

        console.log(`Camada ${layerId} removida`);
    }
}

async function loadLayerByType(layerData) {
    try {
        console.log('Carregando camada:', layerData);
        ...
        switch (layerData.type) {
            case 'crop':
                apiEndpoint = `/api/crop-data/${encodeURIComponent(layerData.crop)}`;
                if (layerData.state) {
                    apiEndpoint += `?state=${layerData.state}`;
                }
                dataKey = 'data';
                break;
            case 'fertilizer':
                apiEndpoint = `/api/fertilizer-data/${encodeURIComponent(layerData.category)}`;
                if (layerData.state) {
                    apiEndpoint += `?state=${layerData.state}`;
                }
                dataKey = 'data';
                break;
            case 'agrotoxicos':
                apiEndpoint = `/api/agrotoxico/data/${encodeURIComponent(layerData.category)}`;
                if (layerData.state) {
                    apiEndpoint += `?state=${layerData.state}`;
                }
                dataKey = 'data';
                break;
            case 'consultoria':
                apiEndpoint = `/api/consultoria/data/${encodeURIComponent(layerData.category)}`;
                if (layerData.state) {
                    apiEndpoint += `?state=${layerData.state}`;
                }
                dataKey = 'data';
                break;
            case 'corretivos':
                apiEndpoint = `/api/corretivos/data/${encodeURIComponent(layerData.category)}`;
                if (layerData.state) {
                    apiEndpoint += `?state=${layerData.state}`;
                }
                dataKey = 'data';
                break;
            case 'despesas':
                apiEndpoint = `/api/despesa/data/${encodeURIComponent(layerData.category)}`;
                if (layerData.state) {
                    apiEndpoint += `?state=${layerData.state}`;
                }
                dataKey = 'data';
                break;
            case 'escolaridade':
                apiEndpoint = `/api/escolaridade/data/${encodeURIComponent(layerData.category)}`;
                if (layerData.state) {
                    apiEndpoint += `?state=${layerData.state}`;
                }
                dataKey = 'data';
                break;
            case 'receitas':
                apiEndpoint = `/api/receita/data/${encodeURIComponent(layerData.category)}`;
                if (layerData.state) {
                    apiEndpoint += `?state=${layerData.state}`;
                }
                dataKey = 'data';
                break;
            case 'revendas':
                // Para revendas, usar os dados do território diretamente
                await loadRevendaLayerFromData(layerData);
                return;
        }
        ...
        if (data.success) {
            const layerAPIData = data[dataKey];
            await loadMunicipalityBoundariesForLayer(layerData, layerAPIData);
        } else {
            console.error('Erro ao carregar dados da camada:', data.error);
            alert('Erro ao carregar dados da camada: ' + data.error);
        }

    } catch (error) {
        console.error('Erro ao carregar camada:', error);
        alert('Erro de conexão ao carregar camada');
    }
}

async function loadMunicipalityBoundariesForLayer(layerData, apiData) {
    try {
        console.log(`Carregando boundaries para camada: ${layerData.name}`);
        ...
        for (const filePath of geoJsonFiles) {
            try {
                ...
                if (response.ok) {
                    ...
                    console.log(`GeoJSON carregado: ${filePath}, ${geoData.features.length} municípios`);
                    break;
                }
            } catch (error) {
                console.log(`Erro ao carregar ${filePath}:`, error);
        ...
        const filteredData = applyStateFilterForLayer(geoData, layerData.state);

        // Calculate min/max for color scaling
        const values = Object.values(apiData)
            .map(item => item[layerData.dataColumn])
            .filter(value => value > 0);
        ...
        const mapLayer = L.geoJSON(filteredData, {
            style: function(feature) {
                return getFeatureStyleForLayer(feature, layerData, apiData, minMax);
            },
            onEachFeature: function(feature, layer) {
                setupFeaturePopupForLayer(feature, layer, layerData, apiData);
            }
        });

        // Store the map layer in the layer data
        layerData.mapLayer = mapLayer;
        layerData.apiData = apiData;
        layerData.minMax = minMax;
        ...
        // Show analytics if it's the first layer
        if (window.activeLayers.length === 1) {
            showAnalyticsSidebar();
            updateAnalyticsSidebar();
        }

        console.log(`Camada ${layerData.name} carregada com sucesso`);

    } catch (error) {
        console.error('Erro ao carregar boundaries da camada:', error);
        alert('Erro ao carregar visualização da camada');
    }
}

async function loadRevendaLayerFromData(layerData) {
    try {
        console.log(`Carregando camada de revenda: ${layerData.name}`);
        ...
        // Get territory municipalities from the layer data
        const territoryMunicipalities = new Set(layerData.territory.map(String));
        ...
        if (filteredFeatures.length === 0) {
            console.warn(`Nenhum município encontrado no território da revenda`);
            return;
        }
        ...
        const mapLayer = L.geoJSON(filteredData, {
            ...
            onEachFeature: function(feature, layer) {
                ...
                popupContent += `
                    <div style="color: ${layerData.color}; font-weight: 600; margin: 8px 0;">
                        <i class="fas fa-store"></i> ${layerData.name.replace('Território - ', '')}
                    </div>
                    <div style="font-size: 12px; color: #666;">
                        Tipo: Território de Revenda<br>
                        Código: ${municipalityCode}
                    </div>
                `;
        ...
        // Store the map layer in the layer data
        layerData.mapLayer = mapLayer;
        layerData.municipalityDetails = filteredFeatures.map(feature => ({
            code: feature.properties.GEOCODIGO || feature.properties.CD_MUN || feature.properties.cd_geocmu || feature.properties.geocodigo,
            name: feature.properties.NOME || feature.properties.NM_MUN || feature.properties.nm_mun || feature.properties.nome,
            state: feature.properties.UF || feature.properties.SIGLA_UF || feature.properties.uf
        }));
        ...
        console.log(`Camada de revenda ${layerData.name} carregada com sucesso`);

    } catch (error) {
        console.error('Erro ao carregar camada de revenda:', error);
        alert('Erro ao carregar visualização da revenda');
    }
}

function getFeatureStyleForLayer(feature, layerData, apiData, minMax) {
    const municipalityCode = feature.properties.GEOCODIGO || 
                           feature.properties.CD_MUN || 
                           feature.properties.cd_geocmu || 
                           feature.properties.geocodigo ||
    ...
    const dataPoint = apiData[municipalityCode];

    if (!dataPoint || !dataPoint[layerData.dataColumn] || dataPoint[layerData.dataColumn] === 0) {
        return {
            fillColor: '#E8E8E8',
            weight: 0.3,
            opacity: 0.8,
    ...
    const value = dataPoint[layerData.dataColumn];
    const color = getColorForValueWithBase(value, minMax.min, minMax.max, layerData.color);
...
function setupFeaturePopupForLayer(feature, layer, layerData, apiData) {
    const municipalityCode = feature.properties.GEOCODIGO || feature.properties.CD_MUN || feature.properties.cd_geocmu || feature.properties.geocodigo;
    const municipalityName = feature.properties.NOME || feature.properties.NM_MUN || feature.properties.nm_mun || feature.properties.nome || 'Nome não disponível';
    const stateUF = feature.properties.UF || feature.properties.SIGLA_UF || feature.properties.uf;
    const dataPoint = apiData[municipalityCode];
    ...
    if (dataPoint && dataPoint[layerData.dataColumn]) {
        const value = dataPoint[layerData.dataColumn];
        const formattedValue = layerData.unit === 'R$' ? 
            `R$ ${value.toLocaleString('pt-BR')}` : 
            `${value.toLocaleString('pt-BR')} ${layerData.unit}`;

        popupContent += `
            <div style="color: ${layerData.color}; font-weight: 600; margin: 8px 0;">
                <i class="fas fa-chart-bar"></i> ${layerData.name}
            </div>
            <div style="font-size: 12px;">
                ${getLayerTypeLabel(layerData.type)}: ${layerData.category || layerData.crop}<br>
...
function getColorForValueWithBase(value, min, max, baseColor) {
    if (value <= 0) value = 1;
...
function updateCombinedLegend() {
    ...
    const visibleLayers = window.activeLayers.filter(layer => layer.visible);
    ...
    window.currentLegendControl.onAdd = function(map) {
        ...
        visibleLayers.forEach((layer, index) => {
            if (index > 0) legendHTML += '<hr style="margin: 10px 0;">';

            legendHTML += `
                <div style="margin-bottom: 10px;">
                    <div style="font-size: 12px; font-weight: 600; margin-bottom: 5px; color: ${layer.color};">
                        ${layer.name}
                    </div>
            `;
            ...
            if (layer.type === 'revendas') {
                legendHTML += `
                    <div style="font-size: 11px; margin-bottom: 5px;">Território de Revenda</div>
                    <div class="legend-item">
                        <div style="width: 18px; height: 3px; background-color: ${layer.color}; display: inline-block; margin-right: 5px; border: 1px solid #ccc;"></div>
                        <span style="font-size: 10px;">Limite do território</span>
                    </div>
                `;
            } else if (layer.minMax) {
                legendHTML += `<div style="font-size: 11px; margin-bottom: 5px;">${layer.unit}</div>`;

                const steps = 4;
                for (let i = 0; i < steps; i++) {
                    let value;
                    if (i === 0) {
                        value = layer.minMax.min;
                    } else if (i === steps - 1) {
                        value = layer.minMax.max;
                    } else {
                        const logMin = Math.log(Math.max(layer.minMax.min, 1));
                        const logMax = Math.log(Math.max(layer.minMax.max, 1));
                        const logValue = logMin + (logMax - logMin) * (i / (steps - 1));
                        value = Math.exp(logValue);
                    }

                    const color = getColorForValueWithBase(value, layer.minMax.min, layer.minMax.max, layer.color);
                    const displayValue = value < 1000 ? 
                        value.toLocaleString('pt-BR', {maximumFractionDigits: 0}) :
                        (value / 1000).toLocaleString('pt-BR', {maximumFractionDigits: 1}) + 'k';
                    ...
                    legendHTML += `
                        <div class="legend-item">
                            <div class="legend-color" style="background-color: ${color}; width: 18px; height: 18px; display: inline-block; margin-right: 5px; border: 1px solid #ccc;"></div>
                            <span style="font-size: 10px;">${displayValue} ${layer.unit}</span>
                        </div>
                    `;
                }

                legendHTML += `
                    <div class="legend-item" style="font-size: 10px; color: #666;">
                        <div class="legend-color" style="background-color: #F5F5F5; width: 18px; height: 18px; display: inline-block; margin-right: 5px; border: 1px solid #ccc;"></div>
                        <span style="font-size: 10px;">Sem dados</span>
                    </div>
                `;
...
function updateAnalyticsSidebar() {
    ...
    visibleLayers.forEach((layer, index) => {
        if (index > 0) analyticsHTML += '<hr style="margin: 20px 0;">';

        analyticsHTML += `
            <div class="analytics-layer-info">
                <div class="analytics-layer-name">${layer.name}</div>
                <div class="analytics-layer-type">${getLayerTypeLabel(layer.type)}</div>
            </div>
        `;

        if (layer.type === 'revendas' && layer.municipalityDetails) {
            const totalMunicipios = layer.municipalityDetails.length;
            const estadosUnicos = [...new Set(layer.municipalityDetails.map(m => m.state))];
            ...
            analyticsHTML += `
                <div class="analytics-stats-grid">
                    <div class="analytics-stat-item">
                        ...
                        <div class="analytics-stat-value">${totalMunicipios}</div>
                    </div>
                    <div class="analytics-stat-item">
                        <div class="analytics-stat-label">Estados</div>
                        <div class="analytics-stat-value">${estadosUnicos.length}</div>
                    </div>
                </div>
            `;
        } else if (layer.apiData && layer.minMax) {
            const values = Object.values(layer.apiData)
                .map(item => item[layer.dataColumn])
                .filter(value => value > 0);
            ...
            analyticsHTML += `
                <div class="analytics-stats-grid">
                    <div class="analytics-stat-item">
                        ...
                        <div class="analytics-stat-value">${total.toLocaleString('pt-BR')}</div>
                    </div>
                    <div class="analytics-stat-item">
                        <div class="analytics-stat-label">Municípios</div>
                        <div class="analytics-stat-value">${municipalitiesCount}</div>
                    </div>
                    <div class="analytics-stat-item">
                        <div class="analytics-stat-label">Média</div>
                        <div class="analytics-stat-value">${average.toLocaleString('pt-BR', {maximumFractionDigits: 0})}</div>
                    </div>
                    <div class="analytics-stat-item">
                        <div class="analytics-stat-label">Máximo</div>
                        <div class="analytics-stat-value">${layer.minMax.max.toLocaleString('pt-BR')}</div>
                    </div>
                </div>
            `;
        }
...
window.loadMunicipalityBoundariesForLayer = loadMunicipalityBoundariesForLayer;
...
async function loadReceitasLayerForLayer(layer) {
        ...
        async function loadMunicipalityBoundariesForRevendasLayer(layer, layerData, minMax) {
            try {
                ...
                // Apply state filter if one is selected (apenas para camadas não-revenda)
                // Para revendas, usamos apenas os dados específicos da revenda, então não aplicamos o filtro de estado aqui
                const filteredData = layer.type !== 'revendas' ? applyStateFilterForLayer(geoData, layer.state) : geoData;
                ...
                const filteredDataGeo = {
                    type: "FeatureCollection",
                    features: filteredFeatures
                };

                // Create a new GeoJSON layer with visible borders
                const mapLayer = L.geoJSON(filteredDataGeo, {
                    style: function(feature) {
...
// Generic layer loading function that handles all layer types
async function loadMunicipalityBoundariesForGenericLayer(layer, layerData, minMax) {
    const geoJsonFiles = [
        '/static/data/brazil_municipalities_all.geojson',
        '/attached_assets/brazil_municipalities_all_1752980285489.geojson',
...
// Add missing functions with proper implementations
async function loadMunicipalityBoundariesForFertilizerLayer(layer, layerData, minMax) {
    try {
        console.log(`Loading fertilizer layer boundaries for: ${layer.name}`);
        ...
        for (const filePath of geoJsonFiles) {
            try {
                ...
                if (response.ok) {
                    ...
                    const mapLayer = L.geoJSON(filteredData, {
                        style: function(feature) {
                            return getFeatureStyleForFertilizerLayer(feature, layer);
                        },
                        onEachFeature: function(feature, mapFeature) {
                            setupFeaturePopupForFertilizerLayer(feature, mapFeature, layer);
                        }
                    });
        ...
        console.error('Erro ao carregar boundaries da camada de fertilizantes:', error);
        alert('Erro ao carregar visualização da camada de fertilizantes');
    }
}

function getFeatureStyleForFertilizerLayer(feature, layer) {
    // Try multiple ways to get municipality code from GeoJSON
    const municipalityCode = feature.properties.GEOCODIGO ||
                           feature.properties.CD_MUN ||
                           feature.properties.cd_geocmu ||
...
function setupFeaturePopupForFertilizerLayer(feature, mapFeature, layer) {
    // Try multiple ways to get municipality info from GeoJSON
    const municipalityCode = feature.properties.GEOCODIGO || feature.properties.CD_MUN || feature.properties.cd_geocmu || feature.properties.geocodigo;
    const municipalityName = feature.properties.NOME || feature.properties.NM_MUN || feature.properties.nm_mun || feature.properties.nome || 'Nome não disponível';
    const stateUF = feature.properties.UF || feature.properties.SIGLA_UF || feature.properties.uf;
    ...
    if (layerData && layerData[layer.dataColumn]) {
        const value = layerData[layer.dataColumn];
        const formattedValue = layer.unit === 'R$' ? 
            `R$ ${value.toLocaleString('pt-BR')}` : 
            `${value.toLocaleString('pt-BR')} ${layer.unit}`;

        popupContent += `
            <div style="color: ${layer.color}; font-weight: 600; margin: 8px 0;">
                <i class="fas fa-flask"></i> ${layer.name}
            </div>
            <div style="font-size: 12px;">
                Fertilizantes: ${layer.category}<br>
                Valor: ${formattedValue}<br>
                Código: ${municipalityCode}
            </div>
        `;
...
function updateAnalyticsCard(layer) {
    console.log('Updating analytics card for layer:', layer.name);
}
...
function filterByState(stateCode) {
    console.log('Filtering by state:', stateCode);
    filterByStateOnMap(stateCode);
}
...
window.loadMunicipalityBoundariesForGenericLayer = loadMunicipalityBoundariesForGenericLayer;
window.loadMunicipalityBoundariesForFertilizerLayer = loadMunicipalityBoundariesForFertilizerLayer;
window.getFeatureStyleForFertilizerLayer = getFeatureStyleForFertilizerLayer;
window.setupFeaturePopupForFertilizerLayer = setupFeaturePopupForFertilizerLayer;
window.generateSequentialColorWithBase = generateSequentialColorWithBase;
window.updateAnalyticsCard = updateAnalyticsCard;
window.toggleAnalyticsCard = toggleAnalyticsCard;
window.calculateCardPosition = calculateCardPosition;
window.repositionAnalyticsCards = repositionAnalyticsCards;
...
// The following functions were defined in the changes but missing from the original code.
// They are added here to ensure the code is complete and runnable.
...
function showLayerConfig(type) {
            ...
            if (type) {
                ...
                if (configPanel) {
                    ...
                    console.log(`Mostrando painel de configuração para: ${type}`);
                } else {
...
// Call setupEventListeners after the DOM is loaded
// This is already handled by the DOMContentLoaded listener above,
// but explicitly calling it here ensures it's available if that listener fails or is moved.
// setupEventListeners(); // Commented out as it's called within DOMContentLoaded

// Placeholder for resetLayerForm if it's used in openLayerModal
function resetLayerForm() {
    console.log('resetLayerForm called - placeholder');
    // Implement actual form reset logic here if needed
}